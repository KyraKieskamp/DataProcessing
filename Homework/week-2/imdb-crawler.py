...
# Discussed some problems with Anneke
'''
...
# to get only numbers from a string
# function obtained from http://stackoverflow.com/questions/10365225/extract-digits-in-a-simple-way-from-a-python-string
def get_num(x):
...
def scrape_movie_page(dom):
    ...
    '''
    # My scraping code
    OV = dom.by_id("overview-top")

    # obtaining title
    title = OV.by_tag("h1.header")[0].by_class("itemprop")[0].content.encode('ascii', 'ignore')

    # obtaining duration
    IB = OV.by_tag("div.infobar")[0]
    dur = IB.by_tag("time")[0].content.encode('ascii', 'ignore')
    duration = str(get_num(dur))

    # obtaining genres
    genres = ""
    for element in IB.by_tag("a"):
        for gen in element.by_tag("span.itemprop"):
            if (len(genres) == 0):
                genres += gen.content.encode('ascii', 'ignore')
            elif (len(gen) >= 0):
                genres += ";" + gen.content.encode('ascii', 'ignore')

    # obtaining directors
    directors = ""
    for element in OV.by_tag("div.txt-block")[0].by_tag("a"):
        for direc in element.by_tag("span.itemprop"):
            if (len(directors) == 0):
                directors += direc.content.encode('ascii', 'ignore')
            elif (len(directors) >= 0):
                directors += ";" + direc.content.encode('ascii', 'ignore')

    # obtaining writers
    writers = ""
    for element in OV.by_tag("div.txt-block")[1].by_tag("a"):
        for writ in element.by_tag("span.itemprop"):
            if (len(writers) == 0):
                writers += writ.content.encode('ascii', 'ignore')
            elif (len(writers) >= 0):
                writers += ";" + writ.content.encode('ascii', 'ignore')

    # obtaining actors
    actors = ""
    for element in OV.by_tag("div.txt-block")[2].by_tag("a"):
        for act in element.by_tag("span.itemprop"):
            if (len(actors) == 0):
                actors += act.content.encode('ascii', 'ignore')
                # inserting ";" for more than one actor
            elif (len(actors) > 0):
                actors += ";" + act.content.encode('ascii', 'ignore')

    # obtaining rating
    rating = OV.by_tag("div.star-box-details")[0].by_tag("span")[0].content.encode('ascii', 'ignore')

    # obtaining n_ratings
    n_ratings = OV.by_tag("div.star-box-details")[0].by_tag("a")[0].by_tag("span")[0].content.encode('ascii', 'ignore')

    # Returning everything of interest for this movie (all strings as specified
